...
//LED pinouts for my testing without a DAC
byte rev = 5;
byte fwd = 6;
byte lft = 9;
byte rgt = 10;
...
void loop() {
  ...
  //Serial.println(in);
  //Ignore this. This was me testing on a LED
  /*
  Serial.print("ud ");
  Serial.println(udSpeed);
  Serial.print("lr ");
  Serial.println(lrSpeed);
  Serial.print("rt ");
  Serial.println(updateSpeedRate);
  
  int udOut = map(udSpeed,MIN_VOLT,MAX_VOLT,-255,255);
  if (udOut < 0) {
    analogWrite(rev, -udOut);
  } else {
    analogWrite(fwd, udOut);
    analogWrite(rev, 0);
  }
  int lrOut = map(lrSpeed,MIN_VOLT,MAX_VOLT,-255,255);
  if (lrOut < 0) {
    analogWrite(lft, -lrOut);
  } else {
    analogWrite(rgt, lrOut);
    analogWrite(lft, 0);
  }
  if ((lrSpeed == CENTRE_VOLT) && (udSpeed == CENTRE_VOLT)) {
    digitalWrite(off, HIGH);
  } else {
